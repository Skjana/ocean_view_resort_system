...
import ocean.view.resort.config.SessionStore;
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        server.start();
        System.out.println("Ocean View Resort running at http://localhost:" + port + " (UI + API at /api, pure Java, no Spring)");
    }
    ...
    private static void handleApi(com.sun.net.httpserver.HttpExchange ex) throws IOException {
        ...
        try {
            Object body = method.equals("POST") || method.equals("PUT") || method.equals("PATCH") ? readBody(ex) : null;
            Object result = route(path, method, query, body, user, auth);
            if (result instanceof BinaryResponse) {
                BinaryResponse br = (BinaryResponse) result;
                ex.getResponseHeaders().set("Content-Type", br.contentType);
                ex.getResponseHeaders().set("Content-Disposition", "attachment; filename=\"" + br.filename + "\"");
                ex.sendResponseHeaders(200, br.data.length);
                ex.getResponseBody().write(br.data);
                ex.close();
                return;
            }
    ...
    private static User resolveUser(String authorization) {
        String token = tokenFromHeader(authorization);
        if (token == null) return null;
        return SessionStore.get(token).orElse(null);
    }

    private static String tokenFromHeader(String authorization) {
    ...
    private static Object route(String path, String method, String query, Object body, User user, String authHeader) {
        // Auth
        if (path.equals("/api/auth/login") && "POST".equals(method)) {
            @SuppressWarnings("unchecked")
            Map<String, String> m = (Map<String, String>) body;
            if (m == null) return new ApiError(400, Map.of("error", "Username and password required"));
            String username = m.get("username");
            String password = m.get("password");
            if (username == null || password == null)
                return new ApiError(400, Map.of("error", "Username and password required"));
            var u = hotelService.authenticate(username.trim(), password);
            if (u.isEmpty()) return new ApiError(401, Map.of("error", "Invalid username or password"));
            String token = SessionStore.create(u.get());
            return Map.of("token", token, "user", mapUser(u.get()));
        }
    ...
    private static class BinaryResponse {
        ...
        BinaryResponse(byte[] data, String contentType, String filename) { this.data = data; this.contentType = contentType; this.filename = filename; }
    }
